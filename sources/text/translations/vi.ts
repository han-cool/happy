...
export const vi = {
    ...
    modals: {
        ...
        deviceLinkedSuccessfully: 'Thiết bị đã được liên kết thành công',
        ...
        developerModeEnabled: 'Chế độ nhà phát triển đã bật',
        developerModeDisabled: 'Chế độ nhà phát triển đã tắt',
        disconnectGithub: 'Ngắt kết nối GitHub',
        disconnectGithubConfirm: 'Bạn có chắc chắn muốn ngắt kết nối tài khoản GitHub?',
    ...
    welcome: {
        ...
        title: 'Nền tảng trợ lý AI',
        subtitle: 'Mã hóa đầu cuối và tài khoản của bạn chỉ được lưu trữ trên thiết bị.',
        createAccount: 'Tạo tài khoản',
        linkOrRestoreAccount: 'Liên kết hoặc khôi phục tài khoản',
    ...
    review: {
        ...
        enjoyingApp: 'Bạn có thích ứng dụng này?',
        feedbackPrompt: 'Chúng tôi rất muốn nghe phản hồi của bạn!',
        yesILoveIt: 'Có, tôi thích nó!',
        notReally: 'Không thực sự'
    },

    items: {
        // Used by Item component for copy toast
        copiedToClipboard: ({ label }: { label: string }) => `${label} đã được sao chép vào clipboard`
    },

    machine: {
        launchNewSessionInDirectory: 'Khởi chạy phiên mới trong thư mục',
        daemon: 'Daemon',
        status: 'Trạng thái',
        stopDaemon: 'Dừng Daemon',
        lastKnownPid: 'PID cuối cùng biết',
        lastKnownHttpPort: 'Cổng HTTP cuối cùng biết',
        ...
        machineGroup: 'Máy tính',
        host: 'Máy chủ',
        machineId: 'ID máy tính',
    ...
    message: {
        ...
        usageLimitUntil: ({ time }: { time: string }) => `Đạt giới hạn sử dụng đến ${time}`,
    ...
    agentInput: {
        ...
        model: {
            ...
            adaptiveUsage: 'Opus đến 50% sử dụng, sau đó Sonnet',
            sonnet: 'Sonnet',
            opus: 'Opus',
        },
        context: {
    ...
    machineLauncher: {
        ...
        showAll: ({ count }: { count: number }) => `Hiện tất cả (${count} đường dẫn)`,
    ...
    tools: {
        fullView: {
            ...
            output: 'Đầu ra',
            error: 'Lỗi',
            completed: 'Công cụ đã hoàn thành thành công',
            noOutput: 'Không có đầu ra được tạo',
            running: 'Công cụ đang chạy...',
            rawJsonDevMode: 'JSON thô (Chế độ phát triển)',
        },
        taskView: {
            initializing: 'Đang khởi tạo agent...',
            moreTools: ({ count }: { count: number }) => `+${count} ${plural({ count, singular: 'công cụ', plural: 'công cụ' })} khác`,
        },
        multiEdit: {
            editNumber: ({ index, total }: { index: number; total: number }) => `Chỉnh sửa ${index} của ${total}`,
        ...
        desc: {
            terminalCmd: ({ cmd }: { cmd: string }) => `Terminal(lệnh: ${cmd})`,
            searchPattern: ({ pattern }: { pattern: string }) => `Tìm kiếm(mẫu: ${pattern})`,
            searchPath: ({ basename }: { basename: string }) => `Tìm kiếm(đường dẫn: ${basename})`,
            fetchUrlHost: ({ host }: { host: string }) => `Lấy URL(url: ${host})`,
            editNotebookMode: ({ path, mode }: { path: string; mode: string }) => `Chỉnh sửa Notebook(tệp: ${path}, chế độ: ${mode})`,
            todoListCount: ({ count }: { count: number }) => `Danh sách việc cần làm(số lượng: ${count})`,
            webSearchQuery: ({ query }: { query: string }) => `Tìm kiếm Web(truy vấn: ${query})`,
            grepPattern: ({ pattern }: { pattern: string }) => `grep(mẫu: ${pattern})`,
    ...
    files: {
        ...
        notUnderGit: 'Thư mục này không thuộc kiểm soát phiên bản git',
    ...
    sessionInfo: {
        ...
        killSession: 'Kết thúc phiên',
        killSessionConfirm: 'Bạn có chắc chắn muốn kết thúc phiên này?',
        happySessionIdCopied: 'ID phiên D3 AI đã được sao chép vào clipboard',
        ...
        claudeCodeSessionIdCopied: 'ID phiên Claude Code đã được sao chép vào clipboard',
        failedToCopyClaudeCodeSessionId: 'Không thể sao chép ID phiên Claude Code',
        metadataCopied: 'Metadata đã được sao chép vào clipboard',
        failedToCopyMetadata: 'Không thể sao chép metadata',
        failedToKillSession: 'Không thể kết thúc phiên',
        connectionStatus: 'Trạng thái kết nối',
        created: 'Đã tạo',
        lastUpdated: 'Cập nhật cuối',
        sequence: 'Trình tự',
        quickActions: 'Hành động nhanh',
        viewMachine: 'Xem máy tính',
        viewMachineSubtitle: 'Xem chi tiết máy tính và phiên',
        ...
        thinkingSince: 'Đang suy nghĩ từ',
    },
    ...
    server: {
        ...
        failedToConnectToServer: 'Không thể kết nối đến máy chủ',
